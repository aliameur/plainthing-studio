import { FC, HTMLProps } from "react";

export const Curve = (props: HTMLProps<SVGSVGElement>) => {
  return (
    <svg
      {...props}
      xmlnsXlink="http://www.w3.org/1999/xlink"
      width="1187"
      height="1001"
      viewBox="-20 -20 1187 1001"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      id="svg1979346536_755"
    >
      <path
        d="M0.66357 395.878C151.766 330.137 486.455 206.496 616.391 237.866C778.81 277.078 535.705 497.898 409.011 567.127C282.318 636.357 175.508 416.402 648.573 377.904C1027.03 347.105 1053.64 623.273 1019.64 765.207"
        stroke='url("#svg1979346536_755_paint0_linear_2_50")'
        strokeWidth="38.86px"
        strokeLinecap="round"
        fill="none"
      ></path>
      <defs>
        <linearGradient
          id="svg1979346536_755_paint0_linear_2_50"
          x1="47.0156"
          y1="317.289"
          x2="842.344"
          y2="-86.5743"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.00824319" stop-color="#FB95DE"></stop>
          <stop offset="0.2696" stop-color="#EC34D9"></stop>
          <stop offset="0.6088" stop-color="#8214E3"></stop>
          <stop offset="0.9847" stop-color="#681CE3"></stop>
        </linearGradient>
        <linearGradient
          id="svg1979346536_755_paint0_linear_2_50"
          x1="47.0156"
          y1="317.289"
          x2="842.344"
          y2="-86.5743"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0.00824319" stop-color="#FB95DE"></stop>
          <stop offset="0.2696" stop-color="#EC34D9"></stop>
          <stop offset="0.6088" stop-color="#8214E3"></stop>
          <stop offset="0.9847" stop-color="#681CE3"></stop>
        </linearGradient>
      </defs>
    </svg>
  );
};
