import React, { HTMLProps } from "react";

interface SmallGridProps extends HTMLProps<SVGSVGElement> {
  variant?: "blue";
}

export const SmallGrid = ({ variant, ...props }: SmallGridProps) => {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 241 245"
      preserveAspectRatio="none"
      width="241"
      height="245"
    >
      <defs>
        <svg
          width="241"
          height="245"
          viewBox="0 0 241 245"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          id="svg-481302386_1968"
        >
          <path
            d="M169.753 56.5801C170.025 54.9287 172.4 54.9287 172.672 56.5801L173.513 61.6881L178.621 62.5294C180.272 62.8015 180.272 65.1758 178.621 65.4478L173.513 66.2892L172.672 71.3972C172.4 73.0486 170.025 73.0486 169.753 71.3972L168.912 66.2892L163.804 65.4478C162.153 65.1758 162.153 62.8015 163.804 62.5294L168.912 61.6881L169.753 56.5801Z"
            fill={variant == "blue" ? "#30BCD6" : "#2F2C34"}
            className="relative"
          ></path>
          <path
            d="M65.9877 56.5801C66.2597 54.9287 68.6341 54.9287 68.9061 56.5801L69.7475 61.6881L74.8555 62.5294C76.5068 62.8015 76.5068 65.1758 74.8555 65.4478L69.7475 66.2892L68.9061 71.3972C68.6341 73.0486 66.2597 73.0486 65.9877 71.3972L65.1463 66.2892L60.0383 65.4478C58.387 65.1758 58.387 62.8015 60.0383 62.5294L65.1463 61.6881L65.9877 56.5801Z"
            fill={variant == "blue" ? "#30BCD6" : "#2F2C34"}
            className="relative"
          ></path>
          <path
            d="M65.9877 151.698C66.2597 150.047 68.6341 150.047 68.9061 151.698L69.7475 156.806L74.8555 157.647C76.5068 157.919 76.5068 160.294 74.8555 160.566L69.7475 161.407L68.9061 166.515C68.6341 168.167 66.2597 168.167 65.9877 166.515L65.1463 161.407L60.0383 160.566C58.387 160.294 58.387 157.919 60.0383 157.647L65.1463 156.806L65.9877 151.698Z"
            fill={variant == "blue" ? "#30BCD6" : "#2F2C34"}
            className="relative"
          ></path>
          <path
            d="M169.753 151.698C170.025 150.047 172.4 150.047 172.672 151.698L173.513 156.806L178.621 157.647C180.272 157.919 180.272 160.294 178.621 160.566L173.513 161.407L172.672 166.515C172.4 168.167 170.025 168.167 169.753 166.515L168.912 161.407L163.804 160.566C162.153 160.294 162.153 157.919 163.804 157.647L168.912 156.806L169.753 151.698Z"
            fill="#2F2C34"
          ></path>
          <line
            x1="67.3361"
            y1="0.00012207"
            x2="67.3361"
            y2="244.586"
            stroke="#2F2C34"
            strokeWidth="1.12539px"
            fill="none"
          ></line>
          <line
            x1="170.872"
            y1="0.00012207"
            x2="170.872"
            y2="244.586"
            stroke="#2F2C34"
            strokeWidth="1.12539px"
            fill="none"
          ></line>
          <line
            x1="-0.00168977"
            y1="63.9601"
            x2="241.452"
            y2="63.235"
            stroke="#2F2C34"
            strokeWidth="1.12539px"
            fill="none"
          ></line>
          <line
            x1="-0.00168977"
            y1="159.244"
            x2="241.452"
            y2="158.518"
            stroke="#2F2C34"
            strokeWidth="1.12539px"
            fill="none"
          ></line>
        </svg>
      </defs>
      <g>
        <path
          d="M169.753 151.698C170.025 150.047 172.4 150.047 172.672 151.698L173.513 156.806L178.621 157.647C180.272 157.919 180.272 160.294 178.621 160.566L173.513 161.407L172.672 166.515C172.4 168.167 170.025 168.167 169.753 166.515L168.912 161.407L163.804 160.566C162.153 160.294 162.153 157.919 163.804 157.647L168.912 156.806L169.753 151.698Z"
          fill="#2F2C34"
        ></path>
        <line
          x1="67.3361"
          y1="0.00012207"
          x2="67.3361"
          y2="244.586"
          stroke={variant == "blue" ? "#30BCD6" : "#2F2C34"}
          strokeWidth="1.12539px"
          fill="none"
        ></line>
        <line
          x1="170.872"
          y1="0.00012207"
          x2="170.872"
          y2="244.586"
          stroke="#2F2C34"
          strokeWidth="1.12539px"
          fill="none"
        ></line>
        <line
          x1="-0.00168977"
          y1="63.9601"
          x2="241.452"
          y2="63.235"
          stroke={variant == "blue" ? "#30BCD6" : "#2F2C34"}
          strokeWidth="1.12539px"
          fill="none"
        ></line>
        <line
          x1="-0.00168977"
          y1="159.244"
          x2="241.452"
          y2="158.518"
          stroke="#2F2C34"
          strokeWidth="1.12539px"
          fill="none"
        ></line>
        <path
          d="M169.753 56.5801C170.025 54.9287 172.4 54.9287 172.672 56.5801L173.513 61.6881L178.621 62.5294C180.272 62.8015 180.272 65.1758 178.621 65.4478L173.513 66.2892L172.672 71.3972C172.4 73.0486 170.025 73.0486 169.753 71.3972L168.912 66.2892L163.804 65.4478C162.153 65.1758 162.153 62.8015 163.804 62.5294L168.912 61.6881L169.753 56.5801Z"
          fill={variant == "blue" ? "#30BCD6" : "#2F2C34"}
        ></path>
        <path
          d="M65.9877 56.5801C66.2597 54.9287 68.6341 54.9287 68.9061 56.5801L69.7475 61.6881L74.8555 62.5294C76.5068 62.8015 76.5068 65.1758 74.8555 65.4478L69.7475 66.2892L68.9061 71.3972C68.6341 73.0486 66.2597 73.0486 65.9877 71.3972L65.1463 66.2892L60.0383 65.4478C58.387 65.1758 58.387 62.8015 60.0383 62.5294L65.1463 61.6881L65.9877 56.5801Z"
          fill={variant == "blue" ? "#30BCD6" : "#2F2C34"}
        ></path>
        <path
          d="M65.9877 151.698C66.2597 150.047 68.6341 150.047 68.9061 151.698L69.7475 156.806L74.8555 157.647C76.5068 157.919 76.5068 160.294 74.8555 160.566L69.7475 161.407L68.9061 166.515C68.6341 168.167 66.2597 168.167 65.9877 166.515L65.1463 161.407L60.0383 160.566C58.387 160.294 58.387 157.919 60.0383 157.647L65.1463 156.806L65.9877 151.698Z"
          fill={variant == "blue" ? "#30BCD6" : "#2F2C34"}
        ></path>
      </g>
    </svg>
  );
};
